...
- Visualizacion de informacion del video (titulo, canal, duracion) antes de descargar.
...
- Sin dependencias externas: no requiere ffmpeg ni Python (yt-dlp se auto-descarga).
...
3.  La app analiza el video con `--dump-json` y muestra titulo, canal y duracion.
...
6.  Al finalizar se muestra un mensaje de exito o error.
...
- Verificar actualizaciones de yt-dlp con la tecla `U`.
- Descargar o actualizar yt-dlp con la tecla `D` cuando haya una version nueva disponible.
...
| U                   | Buscar actualizaciones (en configuracion)   |
| D                   | Descargar/actualizar yt-dlp (en configuracion)|
...
Los presets de audio se muestran separados visualmente en la lista de seleccion.
...
carpetas `yt-dlp/` y `Descargas/`. El binario principal de la app se encuentra en la raiz
del proyecto.
...
- No requiere ffmpeg, Python, ni ninguna otra dependencia externa. yt-dlp se gestiona
  internamente.
- Si el sistema tiene `yt-dlp` en el PATH, la app lo usara directamente sin descargar
  uno propio.
...
| downloadingBinState  | Descarga automatica de yt-dlp si falta           |
...
- **ytdlp_types.go** — Tipos de datos (VideoInfo, QualityPreset, mensajes), carga/guardado
  ...
  y funciones auxiliares (rutas, directorios).
- **ytdlp.go** — Ejecuta yt-dlp como subproceso con `exec.CommandContext`, canaliza la salida
  JSON con el progreso de descarga y la envia como mensajes a Bubble Tea para actualizar la
  barra de progreso.
